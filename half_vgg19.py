import torch
import torch.nn as nn
import torch.distributed as dist
from torch.distributed.device_mesh import DeviceMesh
from torch.utils.data import DataLoader, DistributedSampler
from torchvision import datasets, transforms
from torch.distributed._tensor import Shard, Replicate
from torch.distributed.tensor.parallel import (
    parallelize_module,
    ColwiseParallel,
    RowwiseParallel,
)
import os
import datetime
import subprocess
from torchvision.models import vgg19, vgg16, alexnet

class TrainerTP:
    def __init__(self, model, train_data, test_data, optimizer, device_mesh):
        self.start_time = datetime.datetime.now()
        self.train_data = train_data
        self.global_rank = int(os.environ['RANK'])
        self.local_rank = int(os.environ['LOCAL_RANK'])
        self.device_id = "cuda:" + str(self.local_rank)
        self.model = model.to(self.device_id)
        self.test_data = test_data
        self.optimizer = optimizer
        self.device_mesh = device_mesh

        torch.distributed.barrier()  # Sync all ranks before starting
        print(f"|| Initialized TP Trainer || Rank {self.global_rank} Device {self.device_id}")

    def _run_batch(self, X, y):
        self.optimizer.zero_grad()
        out = self.model(X)
        loss = nn.CrossEntropyLoss()(out, y)
        correct = (out.argmax(1) == y).type(torch.float).sum().item()
        loss.backward()
        self.optimizer.step()
        return correct, loss.item()

    def _run_epoch(self, epoch):
        total_correct = 0
        total_loss = 0.0
        total_samples = 0

        self.model.train()

        for batch, (X, y) in enumerate(self.train_data):
            X, y = X.to(self.device_id), y.to(self.device_id)
            correct, loss = self._run_batch(X, y)

            total_correct += correct
            total_loss += loss * X.size(0)
            total_samples += X.size(0)

            del X, y

        # Reduce across TP ranks
        total_correct_tensor = torch.tensor([total_correct], dtype=torch.float64, device=self.device_id)
        total_samples_tensor = torch.tensor([total_samples], dtype=torch.float64, device=self.device_id)

        dist.all_reduce(total_correct_tensor, op=dist.ReduceOp.SUM)
        dist.all_reduce(total_samples_tensor, op=dist.ReduceOp.SUM)

        accuracy = total_correct_tensor.item() / total_samples_tensor.item()
        return accuracy

    def train(self, max_epochs, early_stop=True):
        early_stop_window = []

        TRAIN_TIME = datetime.datetime.now()
        for epoch in range(max_epochs):
            # checkcuda(self.global_rank)
            

            train_acc = self._run_epoch(epoch)

            if self.global_rank == 0:
                print(f"Epoch {epoch} | Train Accuracy: {train_acc*100:.2f}% | Time: {datetime.datetime.now()-self.start_time}")

            # if self.global_rank == 0:
            acc = self.test()

            # if early_stop:
            #     if len(early_stop_window) == 0:
            #         early_stop_window.append(acc)
            #     else:
            #         last = early_stop_window[-1]
            #         if last * 0.98 <= acc <= last * 1.02:
            #             early_stop_window.append(acc)
            #         else:
            #             early_stop_window = [acc]
            #     if len(early_stop_window) == 3:
            #         print("Early stopping triggered.")
            #         break

            #     print(f"EarlyStopWindow: {early_stop_window}")

    def test(self):
        
        size = len(self.test_data.dataset)
        num_batches = len(self.test_data)

        self.model.eval()
        test_loss, correct = 0, 0

        with torch.no_grad():
            for X, y in self.test_data:
                X, y = X.to(self.device_id, non_blocking=True), y.to(self.device_id, non_blocking=True)
                out = self.model(X)
                test_loss += nn.CrossEntropyLoss()(out, y).item()
                correct += (out.argmax(1) == y).type(torch.float).sum().item()
                del X, y

        # Reduce across TP ranks
        correct_tensor = torch.tensor([correct], dtype=torch.float64, device=self.device_id)
        size_tensor = torch.tensor([size], dtype=torch.float64, device=self.device_id)

        dist.all_reduce(correct_tensor, op=dist.ReduceOp.SUM)
        dist.all_reduce(size_tensor, op=dist.ReduceOp.SUM)

        correct = correct_tensor.item()
        size = size_tensor.item()

        test_loss /= num_batches
        acc = correct / size

        if self.global_rank == 0:
            print(f"Rank {self.global_rank}: Test Accuracy: {(100 * acc):>0.1f}%, Avg loss: {test_loss:>8f} | Time: {datetime.datetime.now()-self.start_time}")

        return acc

def setup(rank, world_size):
    dist.init_process_group("gloo", rank=rank, world_size=world_size)

def cleanup():
    dist.destroy_process_group()

def get_data_loaders(rank, world_size, batch_size):
    transform = transforms.Compose([
        transforms.RandomCrop(32, padding=4),
        transforms.RandomHorizontalFlip(),
        transforms.ToTensor(),
        transforms.Normalize((0.4914, 0.4822, 0.4465), (0.2023, 0.1994, 0.2010))
    ])

    test_transform = transforms.Compose([
        transforms.ToTensor(),
        transforms.Normalize((0.4914, 0.4822, 0.4465), (0.2023, 0.1994, 0.2010))
    ])

    train_dataset = datasets.CIFAR10(
        root='/scratch/pusunuru/data',
        train=True,
        download=False,
        transform=transform
    )

    test_dataset = datasets.CIFAR10(
        root='/scratch/pusunuru/data',
        train=False,
        download=False,
        transform=test_transform
    )

    train_sampler = DistributedSampler(
        train_dataset,
        num_replicas=world_size,
        rank=rank,
        shuffle=True
    )

    test_sampler = DistributedSampler(
        test_dataset,
        num_replicas=world_size,
        rank=rank,
        shuffle=False
    )

    train_loader = DataLoader(
        train_dataset,
        batch_size=batch_size,
        shuffle=True,
        # num_workers=4,  # Reduce CPU bottleneck
        # pin_memory=True 
    )

    test_loader = DataLoader(
        test_dataset,
        batch_size=batch_size,
        shuffle=True,
        # num_workers=4,  # Reduce CPU bottleneck
        # pin_memory=True, 
    )
    
    return train_loader, test_loader, train_sampler

def main():
    rank = int(os.environ['RANK'])
    world_size = int(os.environ['WORLD_SIZE'])
    local_rank = int(os.environ['LOCAL_RANK'])
    batch_size = 64
    print(batch_size)
    setup(rank, world_size)
    torch.cuda.set_device(local_rank)

    # Create 1D device mesh
    device_mesh = DeviceMesh("cuda", list(range(world_size)))
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    # Define the VGG19 model with random weights
    model = vgg19(weights=None, num_classes=10).to(device)
    

    # checkcuda(rank)
    # Model and optimizer

    print(model)
    # checkcuda(rank)
    optimizer = torch.optim.SGD(model.parameters(), lr=0.01, momentum=0.9, weight_decay=5e-4)

    # Data loaders
    train_loader, test_loader, train_sampler = get_data_loaders(rank, world_size, batch_size)

    # Initialize trainer
    trainer = TrainerTP(model, train_loader, test_loader, optimizer, device_mesh)
    
    # Train the model
    trainer.train(max_epochs=30)

    cleanup()

if __name__ == "__main__":
    main()
	